...
- **CLI quick 模式部分失败改为非零退出**：此前只要有任一文件成功即
  exit 0，脚本与 CI 无从感知部分失败；现在 `files_failed > 0` 一律
  exit 1（成功文件照常产出，失败清单见 JSON 摘要）。shell 循环里依赖
  「部分失败继续跑」的脚本不受影响——循环本身不检查退出码；检查退出码
  的调用方会开始收到失败信号（这正是修复目的）。
