...
落地后 1.0 观察期重新计时。剩余收口项（CLI 退出码、capabilities 收窄、
同步命令迁移、分析取消、CMYK APP14 调查等）已全部纳入 1.0 计划的前置
清单，随 0.11.0 一并落地；1.0 落地工作文档（触发条件状态、前置收口
清单、走查、发布 checklist）：[PLAN-1.0.md](PLAN-1.0.md)。
...
- **线性化（Fast Web View）**：
  - 事实：lopdf 0.45.0 源码复核——`SaveOptions::linearize` 字段存在但
    `writer.rs` 从不消费（全库唯一引用是读取侧 `is_linearized` 探测，
    用于把 Catalog 排除出 ObjStm）；仍是空壳，上游无公开路线。
  - 收益侧：线性化服务 HTTP 字节范围的渐进渲染；本工具是桌面压缩器，
    产物在本地消费，上传分享场景由服务端优化器与整文件下载主导，
    首屏收益不可达。零用户请求。
  - 成本侧：ISO 32000 Annex F 的 hint 表 + 首页对象分区 + 与 save_modern
    的 ObjStm 打包交互，多周工程；写错的 hint 比不线性化更糟（阅读器
    对失配容错差），风险落在每一份输出上。
  - 解锁条件：上游实现，或出现真实的网络分发场景需求。
- **JBIG2 随机接入组织（输入）**：
  - 事实：hayro-jbig2 0.3 的 embedded 入口 `new_embedded` 内部走
    `parse_segments_sequential`——只解析顺序组织；支持两种组织的
    是文件格式入口 `new`（Annex D.1/D.2），PDF 流内嵌形态（D.3）未覆盖。
  - 失败模式有界：门禁跳过 → 整图保持原样、分析器如实报告不可行动，
    无损坏风险；主流量产路径（jbig2enc 系、扫描 OCR 栈）的 embedded
    输出以顺序组织为主。
  - 解锁条件：真实语料出现（issue 附样本）→ 向 hayro 提 D.3 随机接入
    支持或本地补段解析。
- **JBIG2 输出编码**：
  - 事实（2026-09-16 复核）：唯一成熟的 Rust 编码器 LegeApp/jbig2enc-rust
    （0.5.3）README 自认"符号匹配路径含改编自 djvulibre 的代码，启用该
    功能的二进制可能带 GPL 影响"、仓库无独立 LICENSE 文件、原仓库已于
    2026-08-23 归档（迁入 Lege monorepo，provenance 更难溯源）；
    tagawa0525/jbig2enc-rs 的 README 已指引用户改用 LegeApp，且其算法
    源自同一 AGPL 原版。MIT 分发 binary 下全部不可用。
  - 成本/收益：自研 T.88 generic-region（无符号字典）约 2-3 周，对 G4
    增益约 10-25%；0.10.0 P0 把近双级默认出口翻成 G4 后，该增益只是
    双级内容尺寸上的一小段进一步缩减。
  - 解锁条件：出现可验证清洁来源的实现（重写符号匹配、有 LICENSE
    文件、provenance 可溯），或双级内容的尺寸投诉使这 10-25% 变关键。
- **简单字体子集化（Type1/PFB、单字节编码简单字体）**：
  - 事实：typst subsetter 只产 CID 键控字体（glyph id 键控、剥 cmap）；
    简单字体经 `/Encoding` 差异映射按单字节码寻址字形——复用 subsetter
    就必须改写内容流的全部文本操作数，Type0 路径"内容流零改写"的
    架构优势恰是子集化的存在理由；裸 PFB Type1 在 Rust 生态没有任何
    subsetter。
  - 收益侧：现代产 PDF 的内嵌字体以 Type0/CID 为主（已覆盖），简单
    字体多为早期产物且字符集小；现护栏对不候选字体整轮放弃，正确性
    无虞。
  - 解锁条件：出现保留简单编码的 Rust subsetter（pyftsubset 的
    simple-font 能力对等物）。
- **2GiB 流式**：
  - 事实：`MAX_INPUT_BYTES = 2 GiB`（compressor.rs），文件与 stdin 两条
    入口都在缓冲前预检并报 `error.inputTooLarge`（e2e 钉死，含 2GiB
    `--ignored` 探针）；峰值内存 ≈2× 输入（0.9.0 管道归还原缓冲后）。
  - 成本侧：PDF 的 xref 是随机访问结构，单遍流式要求增量解析 + 延迟
    物化 + 流式写出——与 0.9.0 刚收口的内存核心 `compress_document`
    架构相抵触，多周重写且全部保真门禁要重锚。
  - 收益侧：>2GiB 的 PDF 极罕见（通常意味着损坏或特殊归档）；失败
    模式是友好报错而非崩溃。零用户请求。
  - 解锁条件：真实大文件场景的用户报告。
- **PDF/A**：
  - 目标冲突：balanced/maximum 默认 `stripMetadata: true`，而 PDF/A
    要求 XMP 元数据——合规输出与压缩诉求同源于"删东西"，只能取一
    （合规模式 = 保守档且全关清理，产品定位随之漂移）。
  - 无验证器：Rust 生态没有 PDF/A 一致性校验器（veraPDF 是 Java）；
    按"诚实性"原则，无法验证的合规声明不可发布。
  - 零用户请求；解锁条件：明确的归档场景需求 + 可用的校验路径。
...
**定位**：1.0 不是功能版本——是兼容性承诺的生效点与对外稳定声明。
全部内容为声明/文档/走查，不携带新功能。

**触发条件（观察期具体化）**：
- 0.10.0（2026-09-16 发布）后 **≥2 周**无兼容性/正确性回报；期间若
  出现 0.10.x patch，以其收敛后重新计时。
- 信号源：GitHub issues / release 评论 / AUR 评论。2026-09-16 盘点：
  三处全零（0.9.0 六天约 15 次资产下载、AUR 双包零票）——无负信号
  但样本小，观察期以时间维度为主。
- 兼容性盘点缺口已清零（0.10.0 P1 的面→测试映射）。
...
2. **默认值不冻结的边界说明**：承诺的是接口/格式兼容，不是行为默认
   值冻结——诚实性修正与 bug 修复仍可改默认，一律走配置迁移模式
   （0.8/0.10 两次先例）并在 CHANGELOG 显著标注。
3. **升级走查**：0.9/0.10 → 1.0 的 updater 路径实测（比较语义已核实
   2026-09-16：updater 按 semver `remote > current` 判定，0.10→1.0
   正常推送）+ 五类安装产物的新装路径（deb/AppImage/dmg/NSIS/zst）。
4. **CHANGELOG 1.0 条目**：成熟度声明 + 0.x 行为变化台账引用（P1 的
