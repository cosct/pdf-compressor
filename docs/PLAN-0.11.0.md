...
**遗留项收口（2026-09-20 全部完成，详见 [PLAN-1.0.md](PLAN-1.0.md) §3）**：

- **6.7 CMYK APP14 调查** ✅：定论引擎现状正确（zune 与 poppler 同在解码内
  反转 Adobe 标记流），矛盾注释改写 + 双夹具像素级钉子入库。
- 4.6 同步命令迁移 ✅（A3，三个阻塞命令已 async 化）。
- jpx-indexed.jp2 再生配方 ✅（B3，确定性再生并替换资产）。
- 变异测试全量 campaign ✅（B2，引擎零漏杀；壳层盲区入 1.x 测试债，
  顺带修复 argv_pdf_paths 双 skip 真 bug）。
