...
**未完成 / 遗留（2026-09-20 拍板：全部迁入 [PLAN-1.0.md](PLAN-1.0.md) §3
前置收口清单，随 0.11.0 发布前落地）**：

- **6.7 CMYK APP14 调查**（PLAN-1.0 §3.2 B1）：造夹具与 poppler 像素级
  对齐后定论；在此之前不动转换逻辑（误改与漏改同险）。
- 4.6 同步命令迁移（PLAN-1.0 §3.1 A3，转正为必做）。
- jpx-indexed.jp2 再生配方（PLAN-1.0 §3.2 B3）。
- 变异测试全量 campaign（PLAN-1.0 §3.2 B2；产物已落 docs/mutation-log.md，
  终态补跑在发布前）。
