...
| 层 | 命令 | 覆盖内容 | 规模 |
| --- | --- | --- | --- |
| 前端单元 | `pnpm test` | 队列/调度/取消（usePdfCompressor）、通知计时、设置面板、右键菜单键盘可达性、App 装配冒烟、en/zh-CN key 树一致性 | 64 项 |
| 引擎单元 | `cargo test -p pdf-core --lib` | 设置夹紧与优先级、分析推荐公式、JPEG 头解析、worker 数量界、resize 行为、目标大小搜索调度数学、CCITT 编解码 round-trip、细节分层 | 92 项 |
| 引擎集成 | `cargo test -p pdf-core --lib`（`pdf/tests.rs`） | 真实 lopdf 构造的 PDF 端到端：往返保文本且缩减、去重、SMask、灰度、G3/G4 转码、目标大小、加密拒绝/owner 解锁/密码解锁、大纲保留、96 用例变异语料不 panic、不写更大输出 | （含于上） |
| CLI 单元 | `cargo test -p pdf-core --bin pdf-compressor-cli` | 参数解析（`parse_size`/`flag_value`/`split_quick_inputs`）、通知文案双语 | 8 项 |
...
| 模糊测试 | `cargo +nightly fuzz run pipeline` | 任意字节跑 analyze+compress+目标大小搜索 | CI 每次 push 60s 冒烟 |
...
```bash
cd crates/pdf-core
cargo +nightly fuzz run pipeline -- -max_total_time=60      # CI 同款冒烟
cargo +nightly fuzz run pipeline -- -max_total_time=600     # 本地长跑
```

目标把任意输入字节写盘后依次跑 analyze → compress → target-size 搜索。不变量：**任何输入都不 panic、不挂起**；损坏图片只产生 skip。崩溃现场在 `fuzz/artifacts/`，用 `cargo +nightly fuzz fmt` 还原最小用例后加进 `pdf/tests.rs` 的变异语料测试。
